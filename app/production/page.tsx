...
const STAGES = ["Awaiting Approval", "Ready for Production", "In Setup", "Printing", "QA", "Completed"];
...
export default function ProductionPage() {
  ...
  useEffect(() => {
    const qJobs = query(collection(db, 'productionJobs'), orderBy('createdAt', 'desc'));
  ...
  const moveJob = async (jobId: string, newStage: string) => {
    try {
      const jobRef = doc(db, 'productionJobs', jobId);
  ...
  return (
    <AppLayout>
      <div className="space-y-6 h-full flex flex-col">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Production Control Center</h1>
          <p className="text-dark-gray/60">Manage screen printing jobs across the shop floor.</p>
        </div>
                            ...
                            <Draggable key={job.id} draggableId={job.id} index={index}>
                              {(provided, snapshot) => (
                                <Card 
                                  ...
                                  className={`cursor-pointer hover:border-royal/50 transition-colors border-l-4 border-l-navy shadow-sm hover:shadow-md ${snapshot.isDragging ? 'shadow-lg ring-2 ring-royal/50' : ''}`}
                                >
                                  <CardContent className="p-4">
                                    <div className="flex justify-between items-start mb-2">
                                      <div className="font-bold text-lg">{order.orderName}</div>
                                      <Badge variant="outline" className="font-mono bg-white">{job.quantity} pcs</Badge>
                                    </div>
